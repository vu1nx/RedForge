"""Domain-facing contracts for replaceable web-crawl providers."""

from dataclasses import dataclass
from enum import StrEnum
from typing import Protocol, cast

from redforge.domain.endpoint import Endpoint
from redforge.domain.host import Host


class WebCrawlProviderStatus(StrEnum):
    """Provider outcome before capability-level status mapping."""

    SUCCESS = "success"
    PARTIAL = "partial"
    FAILURE = "failure"
    UNAVAILABLE = "unavailable"
    ERROR = "error"


@dataclass(frozen=True, slots=True)
class WebCrawlProviderResult:
    """Sanitized immutable crawler evidence and safe parse metadata."""

    endpoints: tuple[Endpoint, ...] = ()
    status: WebCrawlProviderStatus = WebCrawlProviderStatus.SUCCESS
    message: str | None = None
    malformed_record_count: int = 0
    out_of_scope_count: int = 0
    duplicate_count: int = 0
    truncated: bool = False

    def __post_init__(self) -> None:
        endpoints = cast(object, self.endpoints)
        if not isinstance(endpoints, tuple) or not all(
            isinstance(item, Endpoint)
            for item in cast(tuple[object, ...], endpoints)
        ):
            raise TypeError("web crawl endpoints must be an immutable tuple")
        if not isinstance(cast(object, self.status), WebCrawlProviderStatus):
            raise TypeError("web crawl provider status is invalid")
        if self.message is not None and (
            not isinstance(cast(object, self.message), str)
            or not self.message.strip()
        ):
            raise ValueError("web crawl provider message must not be empty")
        for label, value in (
            ("malformed record count", self.malformed_record_count),
            ("out-of-scope count", self.out_of_scope_count),
            ("duplicate count", self.duplicate_count),
        ):
            if (
                not isinstance(cast(object, value), int)
                or isinstance(cast(object, value), bool)
                or value < 0
            ):
                raise ValueError(f"{label} must be a non-negative integer")
        if not isinstance(cast(object, self.truncated), bool):
            raise TypeError("web crawl truncation flag must be boolean")


class WebCrawlProvider(Protocol):
    """Replaceable domain port for crawling approved responsive hosts."""

    def crawl(self, hosts: tuple[Host, ...]) -> WebCrawlProviderResult:
        """Crawl one immutable collection of approved responsive hosts."""
        ...
