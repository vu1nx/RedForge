...
class PipelineBuilder:
    ...
    def build(self, plan: ExecutionPlan) -> Pipeline:
        ...
        steps, output_keys = self._validate(plan)
        missing = next(
            (
        ...
        capabilities = tuple(
            self._factories.create(step.capability_name) for step in steps
        )
        pipeline = Pipeline(output_keys=output_keys)
    ...
    def _validate(
    ...
    ) -> tuple[tuple[ExecutionStep, ...], dict[str, str]]:
        ...
        for step in plan.steps:
            ...
            if (
                step.requires != descriptor.requires
                or step.provides != descriptor.provides
            ):
                raise CapabilityDescriptorMismatchError(
            ...
            if len(descriptor.provides) != 1:
                raise CapabilityDescriptorMismatchError(
                    step.capability_name, "runtime output-state contract"
                )
            output_key = descriptor.provides[0]
            mapped_key = CAPABILITY_OUTPUT_KEYS.get(step.capability_name)
            if mapped_key is not None and mapped_key != output_key:
                raise CapabilityDescriptorMismatchError(
                    step.capability_name, "runtime output-state mapping"
                )
            output_keys[step.capability_name] = output_key
